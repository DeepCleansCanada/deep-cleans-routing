...
export default function IndexPage() {
  ...
  const fetchTomorrowJobs = async () => {
    try {
      ...
      try {
        ...
        throw new Error("Jobs API returned HTML instead of JSON");
      }
      ...
      console.error(err);
      setError(err.message || "Failed to load tomorrow's jobs.");
  ...
  const importTomorrowFromGoogle = async () => {
    try {
      ...
      addLog(
        `Google import finished. fetched=${data.fetched ?? 0}, imported=${data.imported ?? 0}, saved=${data.saved ?? 0}, skipped=${data.skipped ?? 0}`
      );
      ...
      if ((data?.dbError ?? null) !== null) {
        addLog(`DB ERROR: ${data.dbError}`);
      }
